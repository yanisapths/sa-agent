...
function Bar({ used, limit }: { used: number; limit: number | null }) {
  ...
  return (
    <span className="block h-1.5 w-full overflow-hidden rounded-full bg-[#716D65]/15">
      <span
...
export function QuotaMeter({ quota }: { quota: GatewayQuota | null }) {
  ...
  return (
    <Popover.Root>
      <Popover.Trigger className="ml-auto inline-flex cursor-pointer items-center gap-2 self-center rounded-lg px-2 py-1 text-xs transition-colors hover:bg-[#716D65]/10">
        <span className="flex w-28 flex-col gap-1">
          <span className="flex items-baseline justify-between gap-2 text-[11px] text-[#716D65]">
        ...
        <ChevronDown size={13} className="shrink-0 text-[#716D65]" />
      </Popover.Trigger>
      ...
      <Popover.Portal>
        <Popover.Content
          ...
          sideOffset={6}
          collisionPadding={8}
          className="z-50 max-h-[var(--radix-popover-content-available-height)] w-96 overflow-y-auto overscroll-contain rounded-xl border border-[#716D65]/15 bg-white p-4 text-sm shadow-[6px_2px_35px_rgba(0,0,0,0.12)]"
        >
          ...
          <div className="mt-2 flex flex-col gap-1.5">
            <span className="flex items-baseline justify-between text-xs text-[#716D65]">
              <span>
          ...
          {models.length > 0 && (
            <section className="mt-4">
              <h3 className="text-[11px] font-medium uppercase tracking-wide text-[#716D65]">
                By model
              </h3>
              <ul className="mt-1.5 flex flex-col gap-1">
                {models.map((row) => (
                  ...
                  >
                    ...
                    <span className="shrink-0 tabular-nums text-[#716D65]">
